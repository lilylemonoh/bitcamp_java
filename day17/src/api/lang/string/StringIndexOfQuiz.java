...
public class StringIndexOfQuiz {
	...
	public static void main(String[] args) {
		...
		int count = 0; // 감지 횟수
		int nextSearchIdx = 0; // 다음 조회 시 시작지점으로 삼을 인덱스
		int lastSearchedIdx = 0; // 조회 결과 인덱스
		boolean flag = true; // 반복 조회 실행 여부
		System.out.println(sentence + "에서 " + targetWord +"를 찾습니다.");
		while(flag) {
			// 타겟 단어를 마지막으로 감지된 번호로 조회
			lastSearchedIdx = sentence.indexOf(targetWord, nextSearchIdx);
			if( lastSearchedIdx == -1) { // 단어 조회 결과 인덱스가 -1이면
				...
				count++; // 감지된 단어 개수 반영하기
			}
		}
		System.out.println(targetWord + "의 개수는 " + count +"개 입니다.");
